...
func DecodeCleanup(raw json.RawMessage,m Manifest)(Cleanup,error){
	...
	if d.Decode(&c)!=nil{return c,fail};if d.Decode(new(any))!=io.EOF{return c,fail}
	name:="devbox-"+m.Deployment+"-"+m.Owner
	regional:=m.Region+":"+m.Account
	f,s,l:=c.Function,c.Schedule,c.Logs
	if m.SchemaVersion!=6 || c.SchemaVersion!=1 || f.ARN!="arn:aws:lambda:"+regional+":function:"+name+"-cleanup" || f.Runtime!="provided.al2023" || f.Architecture!="x86_64" || !digestRE.MatchString(f.CodeSHA256) || !digestRE.MatchString(f.EnvironmentSHA256) || f.TimeoutSeconds!=180 || f.ServiceTimeoutSeconds!=165 || f.ReservedConcurrency!=1 || f.MemoryMB!=256 || f.AsyncMaxAgeSeconds!=300 || f.AsyncRetryAttempts!=0{return c,fail}
	if s.Name!=name+"-cleanup" || s.GroupName!=s.Name || s.ARN!="arn:aws:scheduler:"+regional+":schedule/"+s.GroupName+"/"+s.Name || s.GroupARN!="arn:aws:scheduler:"+regional+":schedule-group/"+s.GroupName || !slices.Contains([]string{"ENABLED","DISABLED"},s.State) || s.Expression!="rate(5 minutes)" || s.FlexibleWindow!="OFF" || s.MaxAgeSeconds!=300 || s.RetryAttempts!=2 || !digestRE.MatchString(s.InputSHA256){return c,fail}
	if l.Name!="/aws/lambda/"+name+"-cleanup" || l.ARN!="arn:aws:logs:"+regional+":log-group:"+l.Name || !slices.Contains([]int{7,14,30,60,90,120,150,180,365},l.RetentionDays){return c,fail}
	for suffix,r:=range map[string]Role{"cleanup":c.ExecutionRole,"schedule":c.SchedulerRole}{
		if r.ARN!="arn:aws:iam::"+m.Account+":role/"+name+"-"+suffix || r.PolicyName!="devbox-"+suffix || !digestRE.MatchString(r.TrustSHA256) || !digestRE.MatchString(r.PolicySHA256){return c,fail}
	}
	if len(c.Evidence)>0 {
		var capability struct{SchemaVersion int `json:"schema_version"`}
		if json.Unmarshal(c.Evidence,&capability)!=nil || capability.SchemaVersion!=1{return c,fail}
	}
	return c,nil
}
